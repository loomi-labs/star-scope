...
func (cq *ChainQuery) QueryEventListeners() *EventListenerQuery {
...
func (cq *ChainQuery) WithEventListeners(opts ...func(*EventListenerQuery)) *ChainQuery {
	query := (&EventListenerClient{config: cq.config}).Query()
...
func (cq *ChainQuery) loadEventListeners(ctx context.Context, query *EventListenerQuery, nodes []*Chain, init func(*Chain), assign func(*Chain, *EventListener)) error {
	...
	query.Where(predicate.EventListener(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chain.EventListenersColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.chain_event_listeners
		if fk == nil {
			return fmt.Errorf(`foreign-key "chain_event_listeners" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (cq *ChainQuery) loadValidators(ctx context.Context, query *ValidatorQuery, nodes []*Chain, init func(*Chain), assign func(*Chain, *Validator)) error {
	...
	query.Where(predicate.Validator(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chain.ValidatorsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
