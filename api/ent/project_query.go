...
func (pq *ProjectQuery) QueryChannels() *ChannelQuery {
...
func (pq *ProjectQuery) WithChannels(opts ...func(*ChannelQuery)) *ProjectQuery {
	query := (&ChannelClient{config: pq.config}).Query()
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withChannels; query != nil {
		if err := pq.loadChannels(ctx, query, nodes,
			func(n *Project) { n.Edges.Channels = []*Channel{} },
			func(n *Project, e *Channel) { n.Edges.Channels = append(n.Edges.Channels, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *ProjectQuery) loadChannels(ctx context.Context, query *ChannelQuery, nodes []*Project, init func(*Project), assign func(*Project, *Channel)) error {
	...
	query.Where(predicate.Channel(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.ChannelsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.project_channels
		if fk == nil {
			return fmt.Errorf(`foreign-key "project_channels" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "project_channels" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (pq *ProjectQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Project, init func(*Project), assign func(*Project, *User)) error {
	...
	nodeids := make(map[int][]*Project)
	for i := range nodes {
		if nodes[i].user_projects == nil {
			continue
		}
		fk := *nodes[i].user_projects
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_projects" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
