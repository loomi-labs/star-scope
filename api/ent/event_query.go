...
type EventQuery struct {
	...
	withEventListener *EventListenerQuery
	withFKs           bool
...
// QueryEventListener chains the current query on the "event_listener" edge.
func (eq *EventQuery) QueryEventListener() *EventListenerQuery {
	query := (&EventListenerClient{config: eq.config}).Query()
...
func (eq *EventQuery) Clone() *EventQuery {
	...
	return &EventQuery{
		...
		withEventListener: eq.withEventListener.Clone(),
...
// WithEventListener tells the query-builder to eager-load the nodes that are connected to
// the "event_listener" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EventQuery) WithEventListener(opts ...func(*EventListenerQuery)) *EventQuery {
	query := (&EventListenerClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withEventListener = query
	return eq
}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	var (
		...
		_spec       = eq.querySpec()
		loadedTypes = [1]bool{
			eq.withEventListener != nil,
		}
	)
	if eq.withEventListener != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := eq.withEventListener; query != nil {
		if err := eq.loadEventListener(ctx, query, nodes, nil,
			func(n *Event, e *EventListener) { n.Edges.EventListener = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadEventListener(ctx context.Context, query *EventListenerQuery, nodes []*Event, init func(*Event), assign func(*Event, *EventListener)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].event_listener_events == nil {
			continue
		}
		fk := *nodes[i].event_listener_events
	...
	query.Where(eventlistener.IDIn(ids...))
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "event_listener_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
